...
const config = z
  .object({
    ...
    RAG_CHUNK_SIZE: z.string().regex(/^\d+$/).transform(Number),
    RAG_CHUNK_OVERLAP: z.string().regex(/^\d+$/).transform(Number),
    RAG_NUM_OF_CANDIDATES: z.string().regex(/^\d+$/).transform(Number),
    RAG_TOP_K: z.string().regex(/^\d+$/).transform(Number),
    RAG_THRESHOLD: z
      .string()
      .regex(/^0\.\d+$/)
      .transform(Number),
