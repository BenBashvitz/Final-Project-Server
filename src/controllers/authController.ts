...
const setTokens = (
  ...
  const refreshJwtExpirationTimeInMS =
    config.REFRESH_JWT_EXPIRATION_TIME_SECONDS * 1000;
...
const saveTokensAndSendResponse = async (
  user: UserDocument,
  res: Response,
  status: number,
) => {
...
const register = async (req: Request, res: Response) => {
  try {
    ...
    if (error instanceof MongoServerError && error.code === dupKeyErrorCode) {
      return res
        .status(409)
        .send(ResponseErrorMessage.CREDENTIALS_ALREADY_TAKEN);
    }
